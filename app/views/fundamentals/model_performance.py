...
# ─── ZeroEval preprocessing (shared across all sections) ───
if not _ze_df.empty:
    import numpy as np  # noqa: F401 — used implicitly by pandas ops below
...
with st.expander("How models rank when humans judge them head-to-head, vs. how they score on automated benchmarks.", expanded=True):
    ...
    if not df_elo.empty:
        ...
        _elo_floor = round(_elo_min * 0.95)
    ...
    if not df_elo.empty and not _ze_df.empty:
